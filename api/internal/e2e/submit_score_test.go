...
func Test_SubmitScore_NineHole(t *testing.T) {
	...
	// TODO: Update this to account for the third group when we have venue-specific adjustments working.
	require.Len(t, form.Msg.GetForm().GetGroups(), 2)
	...
	adjGroup := form.Msg.GetForm().GetGroups()[1]
	require.Len(t, adjGroup.GetInputs(), 1)
	require.NotNil(t, adjGroup.GetInputs()[0].GetSelectMany())
	...
	// TODO: Enable when we recognize submitted scores and switch to editing mode.
	// form, err = c.GetSubmitScoreForm(context.Background(), requestWithAuth(&apiv1.GetSubmitScoreFormRequest{
	// 	EventKey: testEventKey,
	// 	VenueKey: currentVenueKey,
	// 	PlayerId: playerID.String(),
